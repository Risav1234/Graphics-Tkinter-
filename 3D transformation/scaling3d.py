...
def drawcube(points3d,outline='black'):
	for i in range (len(p3d)):
		points.insert(i,conv3Dto2D(p3d[i][0],p3d[i][1],p3d[i][2]))
...
for i in range(len(p)):
	pointsx.insert(i,scaling(p3dnew[i][0],p3dnew[i][1],p3dnew[i][2]))
print (px)
drawcube(px,'blue')
win.create_text(p[1],text="cube after scaling",font="Times 10 bold")
mainloop()
