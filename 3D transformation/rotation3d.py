...
def drawcube(points3d,outline='black'):
	...
	win.create_polygon(p[0],p[1],p[2],p[3],outline=outline,fill='')
	win.create_polygon(p[4],p[5],p[6],p[7],outline=outline,fill='')
...
drawcube(p3dnew)
win.create_text(p[0],text="Original cube",font="Times 10 bold")
#calculating points after rotation about z axis
px=[]
for i in range(len(p)):
	pointsx.insert(i,rotation(p3dnew[i][0],p3dnew[i][1],p3dnew[i][2]))
print (px)
drawcube(px,'blue')
win.create_text(p[0],text="cube after rotation about z axis",font="Times 10 bold")
mainloop()
